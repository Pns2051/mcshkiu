...
function getReconnectDelay() {
  ...
  // Increased to 30s to prevent IP-limit kicks
...
function createBot() {
  ...
  try {
    ...
    // ✅ RELIABLE AUTO‑RESPAWN (instant + force reconnect if stuck)
    bot.on('death', () => {
      console.log('[Bot] Died – respawning immediately');
      ...
      // If still dead after 3 seconds, force a reconnect
      setTimeout(() => {
        if (bot && bot.entity && bot.health <= 0) {
          console.log('[Bot] Still dead after 3s – forcing reconnect');
          bot.end();
        }
      }, 3000);
    });
...
process.on('SIGTERM', () => {
  console.log('[System] SIGTERM – exiting');
  if (bot) bot.end();
  process.exit(0);
});
process.on('SIGINT', () => {
...
console.log('  Minecraft AFK Bot – Final Edition');
console.log('═'.repeat(45));
