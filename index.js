...
// ============================================================
// DASHBOARD (improved UI, all buttons work)
// ============================================================
app.get('/', (req, res) => {
  ...
  <div class="panel">
    ...
    <div class="card">
      ...
      <div class="footer-note">
        Restart = restart whole Render service. Disconnect = stop bot only.
      </div>
    </div>
...
app.post('/restart', (req, res) => {
  res.json({success:true, message:'Restarting Render service...'});
...
app.post('/toggle', (req, res) => {
  ...
  if (bot && botState.connected) {
    ...
    } catch(e) { console.log('[Config] reload error:', e.message); }
  }
  res.json({success:true});
});
...
setInterval(() => {
  ...
  console.log(`[Memory] ${(mem.heapUsed/1024/1024).toFixed(1)} MB`);
}, 5 * 60 * 1000);
...
function getReconnectDelay() {
  ...
  const base = config.utils['auto-reconnect-delay'] || 15000; // 15s minimum
...
function initializeModules(bot, mcData, defaultMove) {
  ...
  if (config.utils['auto-auth']?.enabled) {
    ...
    bot.on('messagestr', (msg) => {
      ...
      if (m.includes('not registered') || m.includes('register first') || m.includes('/register')) {
        if (!authHandled && attempts < maxAttempts) {
          attempts++;
      ...
      if (m.includes('kicked') || m.includes('time limit') || m.includes('timed out')) {
        authHandled = true;
      }
    });
  ...
  // Move to position (if not circle walking)
