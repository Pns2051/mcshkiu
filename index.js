...
// ============================================================
// DASHBOARD (modern UI, live status, all toggles)
// ============================================================
app.get('/', (req, res) => {
...
function getReconnectDelay() {
  ...
  // Increased base delay to 20 seconds to prevent "already logged in" kicks
  const base = config.utils['auto-reconnect-delay'] || 20000;
...
function createBot() {
  ...
  try {
    ...
    // ✅ RELIABLE AUTO‑RESPAWN (uses bot.respawn() instead of raw packet)
    bot.on('death', () => {
      ...
      setTimeout(() => {
        if (bot && typeof bot.respawn === 'function') {
          ...
          // fallback to raw packet just in case
          if (bot && bot._client) bot._client.write('client_command', { actionId: 0 });
        }
      }, 1000);
    });
...
console.log('  Minecraft AFK Bot – Final Version');
console.log('═'.repeat(45));
